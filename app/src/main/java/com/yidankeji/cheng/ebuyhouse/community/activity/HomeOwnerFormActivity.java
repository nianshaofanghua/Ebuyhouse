...
import com.yidankeji.cheng.ebuyhouse.community.adapter.HintListAdapter;
...
public class HomeOwnerFormActivity extends BaseActivity implements View.OnClickListener, XiangCeRecyclerAdapter.XiangCeOnItemClickListeming, TextWatcher {
    private RecyclerView mPicRecycleView;
    private XiangCeRecyclerAdapter mPicAdapter;
    private EditText et_firstName, et_lastName, et_aptNum, et_addressCity, et_street;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_land_lord_form);
        activity = this;
        initView();
    }
    ...
    public void initView() {
        ...
        isVis = true;
        mLayout = (LinearLayout) findViewById(R.id.ll_parent);
    ...
    public void setData() {
        ...
        mPicAdapter = new XiangCeRecyclerAdapter(HomeOwnerFormActivity.this, mPicList);
        mPicAdapter.setListeming(this);
        mPicRecycleView.setAdapter(mPicAdapter);
        mPicRecycleView.setLayoutManager(new LinearLayoutManager(HomeOwnerFormActivity.this, LinearLayoutManager.HORIZONTAL, false));
        ...
        if(!TextUtils.isEmpty((String)SharedPreferencesUtils.getParam(activity,"city",""))){
            ...
            et_addressCity.setSelection(et_addressCity.getText().length());
        }
            ...
            case R.id.item_selectxiangce_image:
                if (mPicList.size() == position) {
                    if (mPicList.size() < 16) {
                        ...
                        new ImageLogoUtils(HomeOwnerFormActivity.this).getImageLogoDialog(num);
                    } else {

                    }
                } else {
                    PhotoViewUtils.getPhotoView(HomeOwnerFormActivity.this, mPicList, 0);
                }
                break;
            ...
            switch (requestCode) {
                case PictureConfig.CHOOSE_REQUEST:
                    ...
                    if (mPicList.size() + selectList.size() > 16) {
                        ToastUtils.showToast(HomeOwnerFormActivity.this, "photos number too many");
                        return;
                    }
                    LoadingUtils.showDialog(HomeOwnerFormActivity.this);
    ...
    private void submitImagr(final List<LocalMedia> selectList) {
        ...
        new SubmitRoomHttpUtils(HomeOwnerFormActivity.this).compressedImage(selectList, new SubmitRoomHttpUtils.SubmitRoomImageListening() {
            @Override
    ...
    public void submitMessage() {
        ...
        String token = SharedPreferencesUtils.getToken(HomeOwnerFormActivity.this);
        MyApplication.getmMyOkhttp()
                .post()
                .url(Constant.apply_social)
                .addHeader("Authorization", "Bearer " + token)
                .params(parm)
                .enqueue(new NewRawResponseHandler(HomeOwnerFormActivity.this) {
                    @Override
                    public void onSuccess(Object object, int statusCode, String response) {
                        ...
                        if (errorModel.getState() == 1) {
                            Intent intent = new Intent(HomeOwnerFormActivity.this, DataReviewActivity.class);
