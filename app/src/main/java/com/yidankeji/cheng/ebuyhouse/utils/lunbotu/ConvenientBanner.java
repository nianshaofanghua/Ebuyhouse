...
public class ConvenientBanner<T> extends LinearLayout {
	...
	public enum Transformer {
		...
		// 构???器默认也只能是private, 从???保证构造函数只能在内部使用
	 ...
	 * 
	 * @param align
	 *            三个方向：居?? （RelativeLayout.ALIGN_PARENT_LEFT），居中
	...
	public ConvenientBanner setPageTransformer(Transformer transformer) {
		try {
			...
			viewPager.setPageTransformer(true,(PageTransformer) Class.forName(pkName + ".transforms."
	...
	// 触碰控件的时候，翻页应该停止，离??的时候如果之前是??启了翻页的话则重新启动翻??
	@Override
	...
	public ConvenientBanner setOnPageChangeListener(
		...
		// 如果有默认的监听器（即是使用了默认的翻页指示器）则把用户设置的依附到默认的上面，否则就直接设??
